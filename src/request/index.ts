...
instance.interceptors.response.use(
  //这里是处理http请求状态码是200， 接下来要处理业务状态码
  (response: any) => {
    if (response.data) {
      // 当code不是200，并且也不是文件下载接口时。来处理错误
    ...
    request
      .then((response) => {
        ...
        if (response.status === 200) {
          // 针对JSON 请求 → 返回 response.data
          //Blob 请求 → 返回整个 response（就是 Blob 对象
      ...
      /**
       * .catch((error) => { reject(error) }) 做了什么

        .catch() 捕获 请求过程中出现的异常，比如：

        网络错误（断网、超时等）

        axios 抛出的错误（状态码不是 2xx）

        fetch 抛出的异常

        然后 调用封装的 reject(error)，让外部调用方可以在 try/catch 或 .catch() 里拿到错误信息
       */
      .finally(() => {
        if ((loading as NProgress).start) {
